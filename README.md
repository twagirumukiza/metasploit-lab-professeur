...
**Version professeur** : identique au site élève, à une différence près —
tous les chapitres des 5 modules sont accessibles immédiatement, sans
attendre la réussite de l'examen du chapitre précédent (`isUnlocked()`
renvoie toujours `true` dans `app.js`). Pratique pour naviguer librement,
préparer un cours ou vérifier le contenu d'un chapitre avancé sans repasser
tous les examens précédents.

La progression (examens réussis, tentatives) continue d'être enregistrée
normalement dans le navigateur — seul le verrouillage séquentiel est
désactivé.
