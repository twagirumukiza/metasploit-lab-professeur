    ...
    },
    {
      ...
      chapters: [
        ...
        },
        {
          ...
          teacherExtra: {
            fr: `
              <h4>Panorama des vecteurs SET</h4>
              <p>Le menu historique de SET propose plusieurs vecteurs : Spear-Phishing, Website Attack Vectors, Infectious Media Generator, Create a Payload and Listener, Mass Mailer Attack, Teensy USB HID, SMS Spoofing, Wireless Access Point, Third Party Modules. Un vecteur est simplement la voie utilisée pour obtenir une information ou un accès en s'appuyant sur le comportement d'un utilisateur.</p>
              <pre>set@lab:~$ ./set
Social-Engineer Toolkit — SIMULATION LAB
=======================================
...
Select &gt; 2
[LAB] Website vector selected</pre>

              <h4>1. Spear-phishing : dérouler la chaîne pédagogique</h4>
              <p>Faire identifier aux étudiants les six étapes : préparation du scénario → personnalisation du message → envoi simulé → interaction mesurée (ouverture, clic) → mesure de l'événement → rapport expliquant le facteur humain et les mesures de protection. Aucun identifiant réel n'est collecté.</p>
              <pre>[SIMULATION] Spear-Phishing
Mode ............... awareness
Target .............. TRAINING-USER-01
Template ............ security-awareness-demo
[LAB] Message prepared / Delivery simulated
[LAB] Interaction: OPENED
[SAFE] Credentials: NOT COLLECTED</pre>

              <h4>2. Fausse page & tabnabbing : sensibiliser sans collecter</h4>
              <p>Montrer ce qui se passe quand un utilisateur soumet un formulaire sur une page de connexion simulée, puis quand il revient sur un onglet dont l'état a changé pendant son absence. Dans les deux cas, le mot de passe reste masqué (REDACTED) et rien n'est stocké ; seul l'événement est mesuré. Le réflexe défensif à faire ressortir : vérifier le domaine et le contexte avant de saisir une information sensible.</p>
              <pre>[SIMULATION] Credential-Harvesting Awareness
User submits form ............. DETECTED
Password value ................. REDACTED
Stored credential .............. NONE

[SIMULATION] Tabnabbing
Tab A changes state ............ SIMULATED
User returns to Tab A .......... DETECTED</pre>

              <h4>3. Médias amovibles & Teensy USB HID</h4>
              <p>Le générateur de médias infectés et le vecteur Teensy USB HID sont deux vecteurs distincts dans le menu original. L'idée à faire passer : un périphérique USB peut représenter un risque non seulement par les fichiers qu'il contient, mais aussi par la confiance qu'on lui accorde (un clavier « HID » peut être reconnu comme un vrai clavier par le système).</p>
              <pre>[SIMULATION] Infectious Media Awareness
File: awareness-demo.txt
[SAFE] No executable payload / No autorun

[SIMULATION] USB HID Awareness
Device class: keyboard
Keystrokes: NOT SENT</pre>

              <h4>4. SET ↔ Metasploit : séparer la couche humaine de la couche technique</h4>
              <p>SET s'appuie largement sur Metasploit. En classe, on peut garder les deux couches bien séparées : SET produit le scénario d'ingénierie sociale (le « pourquoi » l'utilisateur agit), Metasploit peut ensuite être étudié séparément comme composant technique, dans un laboratoire isolé, sans cible ni payload réels.</p>
              <pre>set@lab:~$ ./set
[LAB] social vector -&gt; training event
msfconsole
msf6 &gt; [LAB] technical layer -&gt; isolated VM
msf6 &gt; [LAB] no payload generated
STATUS: SAFE TRAINING MODE</pre>

              <h4>Exercice à proposer en classe</h4>
              <p>Cas fictif « ACME-LAB » : pour chaque situation, faire identifier vecteur → action de l'utilisateur → événement mesuré → défense recommandée (e-mail de formation → spear-phishing → ouverture/clic → vérifier l'expéditeur ; page de connexion fictive → web → soumission du formulaire → vérifier le domaine ; clé USB inconnue → USB → connexion/ouverture → n'utiliser que des supports autorisés ; périphérique HID → USB HID → détection du périphérique → contrôler les périphériques USB).</p>
              <p><strong>Cadre à rappeler aux étudiants :</strong> uniquement sur des machines, comptes, domaines et participants pour lesquels une autorisation explicite existe ; toutes les consoles ci-dessus sont volontairement simulées et ne contiennent aucune commande capable de compromettre une cible réelle.</p>
            `,
            en: `
              <h4>Overview of SET vectors</h4>
              <p>The historical SET menu offers several vectors: Spear-Phishing, Website Attack Vectors, Infectious Media Generator, Create a Payload and Listener, Mass Mailer Attack, Teensy USB HID, SMS Spoofing, Wireless Access Point, Third Party Modules. A vector is simply the channel used to obtain information or access by relying on a user's behavior.</p>
              <pre>set@lab:~$ ./set
Social-Engineer Toolkit — SIMULATION LAB
=======================================
1) Spear-Phishing Attack Vectors
...
[LAB] Website vector selected</pre>
              ...
              <h4>1. Spear-phishing: walking through the pedagogical chain</h4>
              <p>Have students identify the six steps: scenario preparation → message personalization → simulated delivery → measured interaction (opening, clicking) → event measurement → a report explaining the human factor and protective measures. No real credentials are ever collected.</p>
              <pre>[SIMULATION] Spear-Phishing
Mode ............... awareness
Target .............. TRAINING-USER-01
Template ............ security-awareness-demo
...
[SAFE] Credentials: NOT COLLECTED</pre>
              ...
              <h4>2. Fake page & tabnabbing: raising awareness without collecting anything</h4>
              <p>Show what happens when a user submits a form on a simulated login page, and when they return to a tab whose state changed while they were away. In both cases the password stays masked (REDACTED) and nothing is stored; only the event is measured. The defensive reflex to highlight: check the domain and context before typing anything sensitive.</p>
              <pre>[SIMULATION] Credential-Harvesting Awareness
User submits form ............. DETECTED
Password value ................. REDACTED
Stored credential .............. NONE
...
User returns to Tab A .......... DETECTED</pre>
              ...
              <h4>3. Removable media & Teensy USB HID</h4>
              <p>The infectious media generator and the Teensy USB HID vector are two distinct entries in the original menu. The key idea: a USB device can be risky not only because of the files it carries, but also because of the trust placed in the device itself (an "HID" keyboard can be recognized as a real keyboard by the system).</p>
              <pre>[SIMULATION] Infectious Media Awareness
File: awareness-demo.txt
[SAFE] No executable payload / No autorun
...
Keystrokes: NOT SENT</pre>
              ...
              <h4>4. SET ↔ Metasploit: separating the human layer from the technical layer</h4>
              <p>SET relies heavily on Metasploit. In class, keep the two layers clearly separated: SET produces the social-engineering scenario (the "why" the user acts), while Metasploit can then be studied separately as a technical component, in an isolated lab, with no real target or payload.</p>
              <pre>set@lab:~$ ./set
[LAB] social vector -&gt; training event
msfconsole
msf6 &gt; [LAB] technical layer -&gt; isolated VM
...
STATUS: SAFE TRAINING MODE</pre>
              ...
              <h4>Classroom exercise</h4>
              <p>Fictional case "ACME-LAB": for each situation, have students identify vector → user action → measured event → recommended defense (training email → spear-phishing → opening/clicking → verify the sender; fake login page → web → form submission → check the domain; unknown USB key → USB → plugging in/opening → only use authorized media; HID device → USB HID → device detection → control USB devices).</p>
              <p><strong>Reminder for students:</strong> only on machines, accounts, domains, and participants for which explicit authorization exists; every console above is deliberately simulated and contains no command capable of compromising a real target.</p>
            `
          }
        },
        {
