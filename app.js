...
/* ---------- Rendu : théorie ---------- */
function teacherGuideHtml(chap) {
  const lang = state.lang;
  const keypoints = chap.theory.map((p) => `<li>${p[lang]}</li>`).join("");
  const practiceSteps = chap.practice.map((s) => `<li>${s.instruction[lang]}</li>`).join("");
  const extra = chap.teacherExtra ? `
    <h3>${esc(t("teacher.guide.extraTitle"))}</h3>
    ${chap.teacherExtra[lang]}` : "";
  return `
    <div class="theory-block teacher-guide">
      <p><strong>${esc(t("teacher.guide.objective"))} :</strong> ${chap.intro[lang]}</p>
      <p><strong>${esc(t("teacher.guide.keypoints"))} :</strong></p>
      <ul>${keypoints}</ul>
      <p><strong>${esc(t("teacher.guide.flow"))} :</strong></p>
      <p>${esc(t("teacher.guide.flowPractice"))}</p>
      <ol>${practiceSteps}</ol>
      <p>${esc(t("teacher.guide.flowExam"))} : ${chap.exam.questions.length} ${esc(t("teacher.guide.flowExamText"))}</p>
      ${extra}
    </div>`;
}

function solutionPanelHtml(chap) {
  const lang = state.lang;
  const practiceRows = chap.practice.map((s, i) => {
    const answer = s.accept.find((a) => typeof a === "string") || "";
    return `<li><p class="instruction">${i + 1}. ${s.instruction[lang]}</p><p>${esc(t("teacher.solution.expected"))} : <code>${esc(answer)}</code></p></li>`;
  }).join("");
  const examRows = chap.exam.questions.map((q, i) => `
    <li><p class="exam-q-prompt">${i + 1}. ${q.prompt[lang]}</p><p>${esc(t("exam.correction"))} : ${q.correction[lang]}</p></li>
  `).join("");
  return `
    <div class="callout" id="solutionPanel">
      <h3>${esc(t("teacher.solution.title"))}</h3>
      <p class="section-title">${esc(t("teacher.solution.practice"))}</p>
      <ol>${practiceRows}</ol>
      <p class="section-title">${esc(t("teacher.solution.exam"))}</p>
      <ol>${examRows}</ol>
    </div>`;
}
...
function renderChapterTheory(entry) {
  ...
  document.getElementById("app").innerHTML = `
    ...
    <section class="chapter-content">
      ...
      <h2 class="section-title">${esc(t("teacher.guide.title"))}</h2>
      <button class="btn btn-ghost btn-small" id="toggleGuide">${esc(t(rt.guideVisible ? "teacher.guide.toggleHide" : "teacher.guide.toggleShow"))}</button>
      ${rt.guideVisible ? teacherGuideHtml(chap) : ""}

      <div class="exam-actions" style="margin-top:1.2rem;">
        <button class="btn btn-ghost" id="toggleSolution">${esc(t(rt.solutionVisible ? "teacher.solution.hide" : "teacher.solution.show"))}</button>
      </div>
      ${rt.solutionVisible ? solutionPanelHtml(chap) : ""}
  ...
  document.getElementById("toggleGuide").addEventListener("click", () => {
    rt.guideVisible = !rt.guideVisible; render();
  });
  document.getElementById("toggleSolution").addEventListener("click", () => {
    rt.solutionVisible = !rt.solutionVisible; render();
  });
}
